    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (source == evaluateButton) {   //finish this button event to handle the evaluation and output to infix of the tree
            if (root == null) {
                ...
                String print = BooleanExpressionTreeBuilder.toInfixString(root);
                print += " = ";
                print += root.evaluate();
                JOptionPane.showMessageDialog(this, print, "Result", JOptionPane.INFORMATION_MESSAGE);
            }

        } else if (source == buildTreeButton && postFixField.getText() != null) {
            // COMPLETE ME!!!!!!!!!!
            //Use BooleanExpressionTreeBuilder to build the tree

            root = BooleanExpressionTreeBuilder.buildExpressionTree(postFixField.getText().toCharArray());
        }

        //COMPLETE ME!!!
        //Update the number of nodes label in the tree - if any use BooleaanExpressionTreeBuilder

        numberNodes = BooleanExpressionTreeBuilder.countNodes(root);
        nodeCounterLabel.setText("There are "+numberNodes+" Nodes");
